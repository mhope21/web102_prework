...
function filterFundedOnly() {
    ...
    addGamesToPage(fundedGames, gamesContainer);
...
const summaryMessage = `
  ...
  ${numberOfUnfundedGames > 0
    ? `Currently, ${numberOfUnfundedGames} game${numberOfUnfundedGames !== 1 ? 's' : ''} remain${numberOfUnfundedGames !== 1 ? '' : 's'} unfunded. We need your help to fund these amazing games!`
...
const [topGame, secondTopGame, ...otherGames] = sortedGames;

// create a new element to hold the name of the top pledge game, then append it to the correct element
if (topGame) {
    const topGameName = document.createTextNode(topGame.name);
    firstGameContainer.appendChild(topGameName);
}

// do the same for the runner up item
if (secondTopGame) {
    const secondTopGameName = document.createTextNode(secondTopGame.name);
    secondGameContainer.appendChild(secondTopGameName);
}